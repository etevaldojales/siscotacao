...
<head>
    ...
    <script>
        ...
        function openModal(index, produto, marca, quantidade, obs_produto, unidade) {
            $('#modalProduto').val(produto);
            $('#modalMarca').val(marca);
            $('#modalQuantidade').val(quantidade);
            $('#modalObsProduto').val(obs_produto);
            $('#modalUnidade').val(unidade);
            $('#modalValorUnitario').val($('#valor_unitario_' + index).val());
            $('#modalValorTotal').val($('#vtotal' + index).val());
            $('#modalObservacao').val($('#observacao_' + index).val());
            $('#modalIndex').val(index);
            $('#itemModal').modal('show');
            $('#modalValorUnitario').focus();
        }

        function saveModal() {
            var index = $('#modalIndex').val();
            var valorUnitario = $('#modalValorUnitario').val();
            var valorTotal = $('#modalValorTotal').val();
            var observacao = $('#modalObservacao').val();
            $('#valor_unitario_' + index).val(valorUnitario);
            $('#vtotal' + index).val(valorTotal);
            $('#observacao_' + index).val(observacao);
            $('#itensCotacaoTable tbody tr').eq(index).removeAttr('style').addClass('highlighted-row');
            $('#itemModal').modal('hide');
        }

        function calculaValorTotalModal() {
            var quantidade = parseFloat($('#modalQuantidade').val());
            var valorUnitario = $('#modalValorUnitario').val().replace(',', '.');
            var total = quantidade * parseFloat(valorUnitario);
            $('#modalValorTotal').val(number_format(total, 2, ',', '.'));
        }
                        ...
                        <div class="table-responsive">
                            ...
                            ?>
                                    @forelse ($itens_cotacao as $item)
                                        <tr data-id="{{ $item['id'] }}" style="cursor: pointer;" onclick="openModal({{ $i }}, '{{ $item['produto_nome'] }}', '{{ $item['marca_nome'] }}', '{{ $item['quantidade'] }}', '{{ $item['observacao'] }}', '{{ $item['unidade'] }}')">
                                            <td>{{ $item['produto_nome'] }}</td>
                                            <td>{{ $item['marca_nome'] }}</td>
                                            <td>{{ $item['quantidade'] }}</td>
                                            <td style="display: none;">
                                                <input type="text" name="valor_unitario[]" id="valor_unitario_{{ $i }}" class="form-control"
                                                    oninput="applyMask(this)"
                                            ...
                                            <td style="display: none;">
                                                <input type="text" name="observacao[]" id="observacao_{{ $i }}"
                                                    class="form-control">
                                            </td>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="itemModalLabel">Editar Item</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="modalProduto" class="form-label">Produto</label>
                            <input type="text" class="form-control" id="modalProduto" readonly>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="modalQuantidade" readonly>
                        </div>
                        <div class="mb-3">
                            <label for="modalObsProduto" class="form-label">Obs Produto</label>
                            <input type="text" class="form-control" id="modalObsProduto" readonly>
                        </div>
                        <div class="mb-3">
                            <label for="modalUnidade" class="form-label">Unidade</label>
                            <input type="text" class="form-control" id="modalUnidade" readonly>
                        </div>
                        <div class="mb-3">
                            <label for="modalValorUnitario" class="form-label">Valor Unitário</label>
                            <input type="text" class="form-control" id="modalValorUnitario" oninput="applyMask(this); calculaValorTotalModal()" style="text-align: right">
                        </div>
                        <div class="mb-3">
                            <label for="modalValorTotal" class="form-label">Total</label>
                            <input type="text" class="form-control" id="modalValorTotal" readonly style="text-align: right" readonly>
                        </div>
                        <div class="mb-3">
                            <label for="modalObservacao" class="form-label">Observação</label>
                            <input type="text" class="form-control" id="modalObservacao">
                        </div>
                        <input type="hidden" id="modalIndex">
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
                        <button type="button" class="btn btn-primary" onclick="saveModal()">Salvar</button>
                    </div>
                </div>
            </div>
        </div>
